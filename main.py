...
js_code = 'var timeTable=[["8:20-9:40","8:30-9:50","8:40-10:00"],["10:10-11:20","10:20-11:40","10:40-12:00"],["13:50-15:10","14:00-15:20","14:10-15:30"],["15:40-16:50","15:50-17:10","16:10-17:30"],["18:10-19:30","18:10-19:30","18:10-19:30"],["19:40-21:00","19:40-21:00","19:40-21:00"]];function addTimeShow(){var list=document.querySelectorAll("td");list=Array.from(list);for(var i=list.length-1;i>=0;--i){if(list[i].children.length<1){list.splice(i,1)}}var re=[new RegExp("第一大节"),new RegExp("第二大节"),new RegExp("第三大节"),new RegExp("第四大节"),new RegExp("第五大节"),new RegExp("第六大节")];var idx=0;for(var i=0;i<list.length;++i){var o=list[i];if(idx<5&&re[idx].test(o.innerHTML)){idx++}else{var content=o.children[0].innerHTML;if(new RegExp("敏学楼-D").test(content)||new RegExp("敏学楼-C").test(content)){o.children[0].innerHTML+="<br>"+timeTable[idx][0]}else{if(new RegExp("敏学楼-B").test(content)||new RegExp("微机基础室").test(content)||new RegExp("多媒体教室").test(content)){o.children[0].innerHTML+="<br>"+timeTable[idx][1]}else{o.children[0].innerHTML+="<br>"+timeTable[idx][2]}}}}};addTimeShow();'
